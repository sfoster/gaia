...
marionette('First Time Use > Wifi Hidden Network Test', function() {
  ...
  test('Wi-Fi hidden network show password', function() {
    ...
    // Make sure input isnt auto-corrected
    passwordInput.sendKeys('hairi');
    // waits are to allow keyboard's autocomplete to kick in
    ftu.client.helper.wait(1000);
    passwordInput.sendKeys(' ');
    ftu.client.helper.wait(1000);
    passwordInput.sendKeys('mo');

    // move focus, then check actual value
    ftu.client.findElement('#hidden-wifi-ssid').click();
    assert.equal(passwordInput.getAttribute('value'), 'hairi mo');
  });
